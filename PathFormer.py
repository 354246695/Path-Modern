...
class Model(nn.Module):
    def __init__(self, configs): 
        ...
        # =======  1. 尝试，套用timesnet =======
        # self.projection = nn.Linear(configs.d_model * configs.seq_len * configs.num_nodes, configs.num_class)     # [2*405*61] = 49410 , 2
        # print(f'configs.d_model {configs.d_model}* configs.seq_len {configs.seq_len} * configs.num_nodes {configs.num_nodes} =\n     {configs.d_model * configs.seq_len * configs.num_nodes}, configs.num_class {configs.num_class}')
        # self.projection = nn.Linear(256 * configs.ffn_ratio *102* self.num_nodes, configs.num_class)      # [256*2 * 102 *61] = 49410 , 2
        self.projection = nn.Linear(self.num_nodes *configs.dims[0]* 26, configs.num_class)      # [61 * 256 *52] = 49410 , 2
    ...
    def forward(self, x, padding_mask):
        ...
        for i in range(out.shape[-1]):
            # print(f'\ns输入 m out {i} ============ {out[:, :, :, i].shape}')     #  torch.Size([32, 405, 61, 2])
            out_tem = self.tcn(out[:, :, :, i],  # [B, L, D, S] -> [B, S, D, L]
                            padding_mask, None, None
                            )
            # print(f'\nout_tem {i} ============ {out_tem.shape}')     #  torch.Size([32, 405, 61, 2])
            res.append(out_tem)

        # 沿着周期轴堆叠不同周期的卷积结果 
        # 多尺度特征融合
        # print(f'\n res结果\n   {res.shape}')
        out = torch.cat(res, dim=-1)  # [32,405,32]
        # print(f'\n res cat 结果 out\n   {out.shape}')
