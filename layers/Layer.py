...
# 定义Transformer层的类，继承自nn.Module
class Transformer_Layer(nn.Module):
    def __init__(self, device, d_model, d_ff, num_nodes, patch_nums, patch_size, dynamic, factorized, layer_number, batch_norm,data_len):
        ...
        # 线性变换层
        # self.intra_Linear = nn.Linear(self.patch_nums, self.patch_nums*self.patch_size)   # 原来的
        self.intra_Linear = nn.Linear(self.patch_nums, self.data_len)     # 现在直接改成 数据长度
        # print(f'patch_nums                   {patch_nums}')
        # print(f'patch_size                   {patch_size}')
        # print(f'patch_nums * patch_size      {patch_nums*patch_size}')
        ...
        # patch间注意力机制
        self.inter_patch_attention = Inter_Patch_Attention(self.inter_d_model, self.inter_d_model, 
    ...
    def forward(self, x):
        ...
        intra_out_concat = None  # 初始化内部输出拼接变量
        ...
        x = torch.reshape(x, (x.shape[0] * x.shape[1], 
                            x.shape[2], 
                            x.shape[3] * x.shape[-1]))  # [b*nvar, patch_num, dim*patch_len]

        x = self.emb_linear(x)
        x = self.dropout(x + self.W_pos)  # 融合位置编码并随机失活
        ...
        # ====== 自己添加代码，长度不够补零
        if(inter_out.shape[1] != new_x.shape[1]):
            # print('填充')
            # new_x.shape[1] - inter_out.shape[1]
            inter_out = F.pad(inter_out, (0, 0, 0, 0, 0, new_x.shape[1] - inter_out.shape[1]), "constant", 0)
            # inter_out = F.pad(inter_out, (0, 0, 0, pad_width), "constant", 0)
            # print(f'inter_out {inter_out.size()}')
            

        # # print(new_x.shape[1])
        # print(f'new_x               {new_x.size()}')
        # print(f'intra_out_concat    {intra_out_concat.size()}')
        # print(f'inter_out           {inter_out.size()}')
